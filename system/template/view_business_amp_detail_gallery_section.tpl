[[-view_business_detail_gallery]]
<div id="listing_detail_view_gallery_wrapper_[[*id]]" class="wrapper listing_detail_view_section_wrapper">
    <div class="wrapper listing_detail_view_section_title_wrapper">
        <div class="container listing_detail_view_section_title_container"><h3>Gallery</h3></div>
    </div>
    <div class="wrapper listing_detail_view_section_content_wrapper">
        <div class="listing_detail_view_section_content_container">
            [[$gallery]]
        </div>
    </div>
</div>